...
export default async function HomePage() {
  ...
  if (!profile.is_admin) {
    ...
    return (
      <main className="shell">
        ...
        <div className="merchant-actions"><MerchantOrderPanel products={productList.map((product) => ({ id: product.id, name: product.name, unit: product.unit, base_price: product.finalPrice, stock: product.stock }))} /><OrderList orders={(orders ?? []).map((order) => ({
          id: order.id,
          status: order.status,
  ...
  const { data: orders } = await supabase.from('orders').select('id, merchant_id, status, total, created_at, pickup_time, order_items(product_name, quantity, unit), profiles(username)').order('created_at', { ascending: false })
  const { data: businessHours } = await supabase.from('business_hours').select('*').limit(1)
  const { data: announcements } = await supabase.from('announcements').select('*').eq('is_active', true).order('created_at', { ascending: false })
  ...
  return (
    <main className="shell">
      ...
      <div className="admin-secondary"><AdminOrderPanel initialOrders={(orders ?? []).map((order: any) => ({ id: order.id, merchant_name: order.profiles?.username ?? '商户', status: order.status, total: Number(order.total), created_at: order.created_at, pickup_time: order.pickup_time, items: (order.order_items ?? []).map((item: any) => ({ product_name: item.product_name, quantity: Number(item.quantity), unit: item.unit })) }))} /><BusinessAndNotice businessHours={(businessHours ?? []) as any[]} announcements={(announcements ?? []) as any[]} /><MessageBoard initialMessages={(messages ?? []) as any[]} isAdmin={true} /></div>
    </main>
  )
}
