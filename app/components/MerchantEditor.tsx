...
type Merchant = { id: string; username: string; tag: string | null; merchant_prices: { product_id: string; price: number }[] }
export function MerchantEditor({ merchants, products }: { merchants: Merchant[]; products: Product[] }) {
  ...
  async function saveMerchant(merchant: Merchant) {
    ...
    const { error: profileError } = await supabase.from('profiles').update({ tag: tags[merchant.id] || null }).eq('id', merchant.id)
  ...
  return <div className="merchant-list">{merchants.map((merchant) => <article className="merchant-row" key={merchant.id}><button className="merchant-summary" onClick={() => setExpanded(expanded === merchant.id ? null : merchant.id)}><span className="avatar">{merchant.username.slice(0, 1).toUpperCase()}</span><span><strong>{merchant.username}</strong><small>{merchant.tag ?? '未设置标签'}</small></span><span className="chevron">{expanded === merchant.id ? '−' : '+'}</span></button>{expanded === merchant.id && <div className="price-panel"><label><span>商户标签</span><input value={tags[merchant.id]} onChange={(event) => setTags({ ...tags, [merchant.id]: event.target.value })} placeholder="例如 VIP客户" /></label>{products.map((product) => { const custom = prices[merchant.id]?.[product.id] ?? ''; return <label key={product.id}><span>{product.name}<small>基础价 ¥{Number(product.base_price).toFixed(2)}</small></span><input type="number" step="0.01" placeholder={Number(product.base_price).toFixed(2)} value={custom} onChange={(event) => setPrices({ ...prices, [merchant.id]: { ...prices[merchant.id], [product.id]: event.target.value } })} /></label> })}<button className="button light" onClick={() => saveMerchant(merchant)} disabled={savingId !== null}>{savingId === merchant.id ? '保存中...' : '保存商户信息'}</button></div>}</article>)}{message && <p className={message.includes('失败') ? 'save-message error' : 'save-message'}>{message}</p>}</div>
}
